...
import { Building2, Globe, Loader2, Mail, MapPin, Phone, Plus, Search, Trash2 } from "lucide-react"
...
function LeadStatusBadge({ lead }: { lead: LeadLike }) {
  if (!lead.auditReady) {
    return (
...
function LeadSummary({ lead }: { lead: LeadLike }) {
  return (
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <div className="flex items-end">
              <Button
                ...
                disabled={isSearching || !industry.trim() || !city.trim() || !country.trim()}
              >
            ...
            {searchResults.items.length === 0 ? (
              ...
              <Expandable type="single" collapsible className="divide-y">
                {searchResults.items.map((result, index) => (
                  <ExpandableItem
                    key={`${result.sourceProvider}-${result.sourceId ?? index}`}
                    value={`result-${index}`}
                    className="px-6"
                  >
