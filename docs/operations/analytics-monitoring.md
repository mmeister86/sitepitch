...
Während des Legacy-Backfills sind `reportViews` die autoritative View-Quelle. Eine `reportViewStats`-Zeile mit `viewAggregationState: "pending"` darf Action-Aggregate enthalten, ihre View-Zähler werden von Readern aber noch nicht als vollständig behandelt. Action-Aggregate ohne bestehende Raw-Views starten dagegen direkt als `accurate` mit null Views. Neue Views werden atomar als `includedInStats: true` geschrieben, wenn ihr Aggregate-Zustand bereits eindeutig ist. Trifft ein View auf einen mehrdeutigen pre-fix Zero-Stats-Datensatz mit Legacy-Views, bleibt auch die neue Raw-Zeile unmarkiert und wird vom resumierbaren Backfill exakt mitgezählt. Erst der indexbasierte Finalizer setzt den Audit auf `accurate`.

Alle produktseitigen Reader verwenden denselben Vertrag `{count, capped, pending}`. Pending-Zähler werden in Report, Dashboard, Inbox und Kampagnen als „Wird aktualisiert“ statt als exakte Zahl dargestellt; begrenzte stabile Workspace-Auswertungen tragen ein `+`. Die Dashboard-Gesamtsumme verwendet unabhängig von der Audit-Anzahl genau eine begrenzte Stats- und eine begrenzte Raw-View-Abfrage; nur die höchstens fünf Recent-Audits erhalten eigene Lookups.

Clipboard-Ereignisse sind erfolgsgebunden: `public_link_copied`, `first_shared_report`, `outreach_copied`, zugehörige Rybbit-Events und Success-Toasts entstehen erst, nachdem `navigator.clipboard.writeText` erfolgreich aufgelöst wurde. Bei Ablehnung wird nur ein Fehler angezeigt.
